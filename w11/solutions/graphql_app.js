...
const findCourses = (name) => {
  ...
  return courses.filter((c) => re.test(c.name));
};

const getStudents = (courseName) => {
  return scores.filter((sc) => sc.course === courseName).map((sc) => findStudent(sc.student_id));
};
...
const CourseType = new GraphQLObjectType({
  ...
  fields: {
    ...
    students: {
      ...
      resolve: (course) => getStudents(course.name),
    },
  },
});
